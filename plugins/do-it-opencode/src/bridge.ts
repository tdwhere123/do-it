...
function resolveOverride(value: string, env: NodeJS.ProcessEnv, platform: NodeJS.Platform): string | null {
...
function safeStateSessionKey(sessionId: string): string {
  const unsafe =
    sessionId.length === 0 ||
    sessionId === "." ||
    sessionId === ".." ||
...
function containedStateFile(base: string, sessionKey: string): string | null {
  ...
  for (const filename of ["state.json", "state.kv"]) {
    ...
    try {
      ...
      if (realState.startsWith(`${realBase}${path.sep}`)) return candidate;
    } catch {
...
function terminateProcessTree(child: ReturnType<typeof spawn>, platform: NodeJS.Platform, env: NodeJS.ProcessEnv): void {
  if (!child.pid) return;
  if (platform === "win32") {
...
export async function spawnHook(
  ...
  return await new Promise<HookResult>((resolve) => {
    ...
    child.stdout.on("data", (chunk: Buffer) => {
      if (stdoutBytes >= MAX_HOOK_OUTPUT_BYTES) return;
    ...
    child.stderr.on("data", (chunk: Buffer) => {
      if (stderrBytes >= MAX_HOOK_OUTPUT_BYTES) return;
