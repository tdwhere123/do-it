...
When the task introduces a new dependency, datastore, framework/runtime, or protocol: execute `do-it-planning` § Research-First Comparison. Here, only flag whether a new architectural surface is touched and require the parent to run planning research (search trail, ≥2 candidates, user confirmation via grill). Do not re-derive the full research rules in this lens.
