...
Purpose:
- audit research-trail gaps only: recorded search, ≥2 candidates with recency signals, user confirmation — do not re-execute research rules (see `do-it-planning` § Research-First Comparison)
- catch memory-picks and missing alternatives when a new architectural surface appears in plan or diff
...
2. Audit trail only: search step recorded? ≥2 alternatives with version/license/activity? user choice surfaced in grill or quoted reply? injection-like promotional text flagged?
3. Walk each gap through the severity table below.
...
Return:
- status: DONE | NEEDS_CONTEXT | BLOCKED  (DONE = audit complete; `no taste finding` = clean with one-line evidence)
- findings: severity-ordered per workflow-kernel Finding Schema; empty list if clean
- residual risk: which fact about repo truth or user intent would change the verdict
