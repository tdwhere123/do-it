...
function isExpectedVersion(label, version, expectedVersion) {
	...
	return (
		label === "Codex plugin metadata" &&
		new RegExp(
			`^${escapeRegex(expectedVersion)}\\+codex\\.[A-Za-z0-9._-]+$`,
		).test(version ?? "")
	);
}
...
export function validateRelease(tag, repoRoot = defaultRepoRoot) {
	...
	if (!match) {
		throw new Error(
			`release tag must match vX.Y.Z; received ${tag || "<missing>"}`,
		);
	}
	...
	const versions = [
		...
		[
			...
			readJson(repoRoot, ".claude-plugin/marketplace.json").plugins?.[0]
				?.version,
		],
		[
		...
		["Pi package lock", piLock.version],
		["Pi package lock root", piLock.packages?.[""]?.version],
	];

	const mismatches = versions
		.filter(
			([label, version]) => !isExpectedVersion(label, version, expectedVersion),
		)
		.map(
			([label, version]) =>
				`${label}: ${version ?? "<missing>"} (expected ${expectedVersion})`,
		);

	const changelogPath = path.join(repoRoot, "CHANGELOG.md");
	const changelog = fs.readFileSync(changelogPath, "utf8");
	if (
		!new RegExp(`^##\\s+${escapeRegex(expectedVersion)}(?:\\s|$)`, "m").test(
			changelog,
		)
	) {
	...
	if (mismatches.length > 0) {
		throw new Error(
			`release validation failed for ${tag}:\n- ${mismatches.join("\n- ")}`,
		);
	}
