---
name: do-it-handbook
description: "Problem: every new session re-derives the same architecture, glossary, code map, and review protocol from scratch because the project has no durable handbook the agent can rely on. Fix: bootstrap a `.do-it/handbook/` skeleton — invariants, architecture, code-map, glossary, backlog, runtime-status, maintenance, task-card template, and three workflow files — that downstream do-it skills read instead of re-deriving."
---
