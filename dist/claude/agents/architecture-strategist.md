...
description: "Use through do-it-decide when architecture choices need a read-only foundation, extension, boundary, and stage-closure lens."
---
...
When the task introduces a new dependency, datastore, framework/runtime, or protocol: flag the new architectural surface and require the parent to use `do-it-decide` Research-First guidance (search trail, ≥2 candidates, and a decision only where preference changes the route). Do not re-derive the full research rules in this lens.
...
Optional lenses (apply only when parent asks or task-fit is obvious; keep each to a few bullets):
- scan / delivered risk: ownership boundaries; dependency direction; unwired or unused surfaces; coupling; rollout / migration; failure isolation; observability at the boundary
- research-first audit: new dep / datastore / framework / protocol — recorded search? ≥2 candidates with version/license/activity? user confirmation? treat memory-picks as Blocking
- ops / SRE: deploy path and blast radius; one-button vs manual rollback; metrics/alerts for on-call; migration safety; on-call burden delta
...
- broad redesign for a scoped task unless the parent requested that level
