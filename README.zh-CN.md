...
`do-it` 仅以 **Cursor 插件市场** 形式分发，没有 `do-it install --target=cursor`。
...
Cursor 适配器为中等 hook 深度：`sessionStart` 引导、`beforeSubmitPrompt`
路由/grill/stance、`preToolUse` 计划闸、`postToolUse` / `afterFileEdit` 旁路
`write-quality-lint`、`stop` 验证闸。详见
...
`do-it` 提供 OpenCode TypeScript 插件：transform 引导 + 选择性 bash 桥接
（pretool、write-quality、verification）。

将 `plugins/do-it-opencode/opencode.json.template` 复制到你的 OpenCode 配置，
并把插件路径指向本仓库的 `plugins/do-it-opencode/`。说明见
