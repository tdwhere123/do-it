...
- 可移植的 Codex agent 定义：代码路径映射、计划挑战、正确性审查、架构审查、
  ...
  **四个产品 / UX / 终端用户 / 运维 persona（`ceo-reviewer`、`ux-designer`、
  `end-user-advocate`、`ops-sre`），在 Claude 路径下默认运行 Sonnet**。
...
flowchart TD
    ...
    R --> M{需要多视角<br/>脑暴吗}
    M -- 需要 --> BR[do-it-brainstorm<br/>CEO / UX / 终端用户 / Ops<br/>并行，Sonnet，read-only]
    M -- 不需要 --> G
    BR --> G{前提稳吗}
...
2. 涉及产品 / UX / 发布相关的工作，`do-it-brainstorm` 在 grill 之前并行派
   出四个 read-only persona（CEO / UX / 终端用户 / Ops），让 grill 拿到的是
   已经被外部视角拓宽过的前提。Light 不跑 brainstorm；Standard 跑 2-3 个最
   相关的 persona；Heavy 全部四个一起跑。
3. 当存在 brainstorm 产物时，`do-it-grill` 进入收敛模式：把每个 persona 的
   「一个问题」当成候选前提排序、逐项核验或留给用户决策，而不是重新发散。
...
`do-it 0.6.0` 在 0.5.1 之上加了四件事，全部按触发词或 tier 显式启用——既
不会打扰 Light tier，也不会打断已 grill 过的 Standard 流程。

**grill 之前先 brainstorm。** 新的 `do-it-brainstorm` skill 会并行派出四个
read-only persona 子代理：`ceo-reviewer`（产品 / 商业）、`ux-designer`（UI
/ 交互）、`end-user-advocate`（真实使用体验）、`ops-sre`（部署 / 观测 / 回
滚）。每个在 Claude 路径下跑 Sonnet、严格 ≤ 150 行返回，把各自的「一个问题」
汇总进 `.do-it/brainstorm/<task>.md`。固定形态（四个有名字的视角 + 固定
token 预算）就是它的价值——这不是一个自由发挥的「what if」便笺。

**grill 改成做收敛而不是重新发散。** 当 brainstorm 文件存在且 `status: open`
时，`do-it-grill` 会把每个 persona 的「一个问题」拉成候选前提，按「错了影
响最大」排序，逐项核验或留作 user decision，最后把 brainstorm 的 `status:
open` 翻成 `converged`。Light tier 仍然走原来的单线程 grill。
