...
每个被委派的 slice 都要锁定完整的 **Delegation Contract**：

| 字段 | 锁定的内容 |
|---|---|
| `tier + lens` | Light / Standard / Heavy，以及被分配的视角。 |
| `scope + non-goals` | 边界明确的产出，以及明确不做什么。 |
| `write ownership + restricted paths` | 允许和禁止编辑的准确路径（或明确只读）。 |
| `facts to verify` | 子 agent 动手之前必须确认的具体声明。 |
| `proof target` | 用什么证据证明被委派的结果。 |
| `stop condition` | 触发子 agent 收尾的具体事件。 |
| `return schema` | 简洁的 `DONE | NEEDS_CONTEXT | BLOCKED` 回报。 |

父 agent 必须把这些字段直接写进 prompt。可移植 agent 不假定仓库里的相对路径
指令文件一定可用；字段缺失时必须在读写文件前返回 `NEEDS_CONTEXT` 并列出缺项。
...
| 真相平面 | 本仓库可以声明的内容 |
|---|---|
| 源码 / 包元数据 | 当前 checkout 声明 `0.14.0`、8 个用户可运行 skill + 1 个生成式发现入口、10 个 agent。 |
...
8 个用户可运行 skill、1 个生成式 `_index.md` 发现入口、10 个 agent，以及插件内 hooks。
...
|---|---|---|---|
| Codex / Claude / Cursor / OpenCode | 8 个 — 5 核心 + 3 扩展 | 1 个生成式 `_index.md` 入口（不是第九个 skill） | 10 个 |
...
- 四个宿主的插件内 hooks：router、仅 Heavy 的 `grill-prompt`、
  ...
  Codex / Claude / Cursor 使用阻断式的启发式 Stop 检查，OpenCode 只能在 idle
  时做软提醒；任何宿主都不再注册 `grill-pretool` 计划闸。
...
plugins/do-it-cursor/   生成后的 Cursor plugin bundle（完整 8 个 skill）
