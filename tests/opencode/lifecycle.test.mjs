...
test("fake host exercises config, bootstrap, hooks, idle notification, and cleanup", async () => {
  ...
  try {
    ...
    assert.match(config.agent["code-mapper"].prompt, /Act as a read-only path mapper/);
    ...
    assert.match(messages[0].parts[0].text, /direct user intent wins over hook heuristics/);
    assert.match(messages[0].parts[0].text, /Bundled subagents are visible in the host agent list/);
...
function setRecorderEnabled(cwd, sessionID = "feedback-control") {
  ...
  const result = spawnSync("bash", [hook], {
    cwd,
    input: JSON.stringify({ session_id: sessionID, cwd, prompt: "/do-it-retrospective on" }),
    encoding: "utf8"
  });
  assert.equal(result.status, 0, result.stderr);
  const config = path.join(cwd, ".do-it", "runtime", "retrospective", "config.json");
  assert.equal(JSON.parse(fs.readFileSync(config, "utf8")).enabled, true, "control command should enable the recorder");
}
...
test("feedback capture uses OpenCode parentage and never records a child session", async () => {
  const rootCwd = fs.mkdtempSync(path.join(os.tmpdir(), "doit-opencode-feedback-root-"));
  const childCwd = fs.mkdtempSync(path.join(os.tmpdir(), "doit-opencode-feedback-child-"));
  const previousData = process.env.OPENCODE_DATA;
  ...
  try {
    ...
    await rootHooks["chat.message"](
      ...
      { parts: [{ type: "text", text: "do-it behavior is unexpected: delegation was missed" }] }
    );
    assert.equal(rootHost.calls.get, 1, "enabled root recorder should query session parentage");
    const rootEvents = path.join(rootCwd, ".do-it", "runtime", "retrospective", "events.jsonl");
    assert.ok(fs.existsSync(rootEvents), "root session should be eligible for opt-in feedback capture");
    ...
    assert.equal(childHost.calls.get, 1, "enabled child recorder should query session parentage");
    const childEvents = path.join(childCwd, ".do-it", "runtime", "retrospective", "events.jsonl");
    assert.equal(fs.existsSync(childEvents), false, "child session must not be recorded");
  } finally {
    if (previousData === undefined) delete process.env.OPENCODE_DATA;
    else process.env.OPENCODE_DATA = previousData;
